...
for _, strategy in helpers.each_strategy() do
  describe("helpers [#" .. strategy .. "]: assertions and modifiers", function()
    ...
    describe("await_file_contents()", function()
      local function time()
        ngx.update_time()
        return ngx.now()
      end
      ...
      it("returns the file contents when the file is readable and non-empty", function()
        ...
        assert.equals("test", helpers.await_file_contents(fname))
      end)
      ...
      it("waits for the file if need be", function()
        ...
        ngx.timer.at(0, function()
          ...
          ok, res = pcall(helpers.await_file_contents, fname, timeout)
      ...
      it("doesn't wait longer than the timeout in the failure case", function()
        ...
        ngx.timer.at(0, function()
          ...
          ok, err = pcall(helpers.await_file_contents, fname, timeout)
        ...
        assert.falsy(ok, "expected await_file_contents to fail")
        assert.not_nil(err)
      ...
      it("raises an assertion error if the file does not exist", function()
        assert.error_matches(function()
          helpers.await_file_contents("/i/do/not/exist", 0)
      ...
      it("raises an assertion error if the file is empty", function()
        ...
        assert.error_matches(function()
          helpers.await_file_contents(fname, 0)
